...
class RecordListFragment : Fragment() {
    ...
    private lateinit var viewModelFactory: ShowRecordsViewModel.Factory
    ...
    private fun onItemLongTap(item: WeightItemEntity) {
        val value = requireContext().getSharedPreferences(PREFERENCES_NAME, Context.MODE_PRIVATE)
            .getString(
                PREFERENCE_KEY.LONG_TAP.name,
                resources.getStringArray(R.array.setting_long_tap)[0]
            )
        when (value) {
            getString(R.string.settings_lt_show_menu) -> showMenuDialog(item)
            getString(R.string.settings_lt_edit) -> RxBus.publish(
                InputFragmentStartEvent(
